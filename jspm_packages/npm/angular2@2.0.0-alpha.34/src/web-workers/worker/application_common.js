...
function _injectorBindings(appComponentType, bus, initData) {
  ...
  return [di_1.bind(application_tokens_1.appComponentTypeToken).toValue(appComponentType), di_1.bind(application_tokens_1.appComponentRefPromiseToken).toFactory(function(dynamicComponentLoader, injector) {
    ...
    return ref.instance;
  }, [application_tokens_1.appComponentRefPromiseToken]), di_1.bind(life_cycle_1.LifeCycle).toFactory(function(exceptionHandler) {
    return new life_cycle_1.LifeCycle(null, lang_1.assertionsEnabled());
  }, [exception_handler_1.ExceptionHandler]), serializer_1.Serializer, di_1.bind(application_1.WorkerMessageBus).toValue(bus), di_1.bind(broker_1.MessageBroker).toFactory(function(a, b, c) {
    return new broker_1.MessageBroker(a, b, c);
  }, [application_1.WorkerMessageBus, serializer_1.Serializer, ng_zone_1.NgZone]), renderer_1.WorkerRenderer, di_1.bind(api_1.Renderer).toAlias(renderer_1.WorkerRenderer), renderer_1.WorkerCompiler, di_1.bind(api_1.RenderCompiler).toAlias(renderer_1.WorkerCompiler), di_1.bind(api_2.ON_WEBWORKER).toValue(true), render_view_with_fragments_store_1.RenderViewWithFragmentsStore, render_proto_view_ref_store_1.RenderProtoViewRefStore, proto_view_factory_1.ProtoViewFactory, view_pool_1.AppViewPool, di_1.bind(view_pool_1.APP_VIEW_POOL_CAPACITY).toValue(10000), view_manager_1.AppViewManager, view_manager_utils_1.AppViewManagerUtils, view_listener_1.AppViewListener, compiler_1.Compiler, compiler_1.CompilerCache, view_resolver_1.ViewResolver, di_1.bind(change_detection_1.Pipes).toValue(change_detection_1.defaultPipes), di_1.bind(change_detection_1.ChangeDetection).toClass(bestChangeDetection), directive_resolver_1.DirectiveResolver, change_detection_1.Parser, change_detection_1.Lexer, di_1.bind(exception_handler_1.ExceptionHandler).toFactory(function() {
