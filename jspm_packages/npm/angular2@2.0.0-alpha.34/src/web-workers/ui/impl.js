...
var WebWorkerMain = (function() {
  ...
  WebWorkerMain.prototype.attachToWorker = function(bus) {
    ...
    this._bus.source.addListener(function(message) {
      _this._handleWorkerMessage(message);
    });
  };
  ...
  WebWorkerMain.prototype._sendWorkerMessage = function(type, value, id) {
    this._bus.sink.send({
      'type': type,
      'id': id,
      'value': value
  ...
  WebWorkerMain.prototype._wrapWorkerPromise = function(id, promise, type) {
    ...
    async_1.PromiseWrapper.then(promise, function(result) {
      try {
        _this._sendWorkerMessage("result", _this._serializer.serialize(result, type), id);
      } catch (e) {
        lang_1.print(e);
      }
    }, function(error) {
      _this._sendWorkerError(id, error);
    });
  };
  WebWorkerMain = __decorate([di_1.Injectable(), __metadata('design:paramtypes', [api_1.RenderCompiler, api_1.Renderer, render_view_with_fragments_store_1.RenderViewWithFragmentsStore, serializer_1.Serializer, anchor_based_app_root_url_1.AnchorBasedAppRootUrl])], WebWorkerMain);
...
var EventDispatcher = (function() {
  ...
  EventDispatcher.prototype.dispatchRenderEvent = function(elementIndex, eventName, locals) {
    ...
    switch (e.type) {
      ...
      case "change":
      case "blur":
        serializedEvent = event_serializer_1.serializeEventWithValue(e);
        break;
      case "abort":
      case "afterprint":
      case "beforeprint":
