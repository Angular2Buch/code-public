...
var RouterOutlet = (function() {
  ...
  RouterOutlet.prototype.commit = function(instruction) {
    ...
    if (instruction.reuse) {
      ...
      next = this.deactivate(instruction).then(function(_) {
        return _this._activate(instruction);
      });
    }
  ...
  RouterOutlet.prototype._activate = function(instruction) {
    ...
    var bindings = di_1.Injector.resolve([di_1.bind(instruction_1.RouteParams).toValue(new instruction_1.RouteParams(instruction.params())), di_1.bind(routerMod.Router).toValue(this.childRouter)]);
  ...
  RouterOutlet.prototype.canReuse = function(nextInstruction) {
    var result;
    if (lang_1.isBlank(this._currentInstruction) || this._currentInstruction.component != nextInstruction.component) {
      result = false;
      ...
      result = nextInstruction == this._currentInstruction || collection_1.StringMapWrapper.equals(nextInstruction.params(), this._currentInstruction.params());
    }
  ...
  RouterOutlet.prototype._reuse = function(instruction) {
    var previousInstruction = this._currentInstruction;
    this._currentInstruction = instruction;
    return async_1.PromiseWrapper.resolve(route_lifecycle_reflector_1.hasLifecycleHook(hookMod.onReuse, this._currentInstruction.component) ? this._componentRef.instance.onReuse(instruction, previousInstruction) : true);
  };
  RouterOutlet.prototype.deactivate = function(nextInstruction) {
    var _this = this;
    return (lang_1.isPresent(this.childRouter) ? this.childRouter.deactivate(lang_1.isPresent(nextInstruction) ? nextInstruction.child : null) : async_1.PromiseWrapper.resolve(true)).then(function(_) {
      if (lang_1.isPresent(_this._componentRef) && lang_1.isPresent(_this._currentInstruction) && route_lifecycle_reflector_1.hasLifecycleHook(hookMod.onDeactivate, _this._currentInstruction.component)) {
