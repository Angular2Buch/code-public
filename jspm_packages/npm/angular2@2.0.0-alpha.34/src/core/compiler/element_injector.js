...
var DirectiveBinding = (function(_super) {
  ...
  DirectiveBinding.createFromBinding = function(binding, ann) {
    ...
    var resolvedViewBindings = ann instanceof annotations_1.Component && lang_1.isPresent(ann.viewBindings) ? di_1.Injector.resolve(ann.viewBindings) : [];
...
var ElementInjector = (function(_super) {
  ...
  ElementInjector.prototype._addViewQueries = function(host) {
    if (lang_1.isPresent(host._query0) && host._query0.originator == host && host._query0.query.isViewQuery)
      this._addViewQuery(host._query0);
    if (lang_1.isPresent(host._query1) && host._query1.originator == host && host._query1.query.isViewQuery)
      this._addViewQuery(host._query1);
    if (lang_1.isPresent(host._query2) && host._query2.originator == host && host._query2.query.isViewQuery)
  ...
  ElementInjector.prototype.unlink = function() {
    ...
    this.remove();
    var nonViewQueries = collection_1.ListWrapper.filter(queriesToUpdate, function(q) {
      return !q.query.isViewQuery;
    });
    collection_1.ListWrapper.forEach(nonViewQueries, function(q) {
      return q.update();
    });
  };
...
var ElementInjectorDynamicStrategy = (function() {
  ...
  ElementInjectorDynamicStrategy.prototype.hydrate = function() {
    ...
    for (var i = 0; i < p.keyIds.length; i++) {
      if (p.bindings[i] instanceof DirectiveBinding && lang_1.isPresent(p.keyIds[i]) && inj.objs[i] === di_1.undefinedValue) {
