...
import { NoAnnotationError, MixingMultiBindingsWithRegularBindings, InvalidBindingError } from './exceptions';
...
export class BindingBuilder {
    ...
    toClass(type) {
        if (!isType(type)) {
            throw new BaseException(`Trying to create a class binding but "${stringify(type)}" is not a class!`);
        }
    ...
    toFactory(factory, dependencies) {
        if (!isFunction(factory)) {
            throw new BaseException(`Trying to create a factory binding but "${stringify(factory)}" is not a function!`);
        }
 ...
 */
export function resolveFactory(binding) {
    var factoryFn;
    var resolvedDeps;
