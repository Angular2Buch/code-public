...
import { ViewType } from 'angular2/src/core/render/api';
...
export let ProtoViewFactory = class {
    ...
    createHost(compiledHostTemplate) {
        ...
        if (isBlank(result)) {
            ...
            result =
                new AppProtoView(templateData.commands, ViewType.HOST, true, templateData.changeDetectorFactory, null, new ProtoPipes(new Map()));
    ...
    _createComponent(cmd) {
        ...
        if (isBlank(nestedProtoView)) {
            ...
            nestedProtoView = new AppProtoView(compiledTemplateData.commands, ViewType.COMPONENT, true, compiledTemplateData.changeDetectorFactory, null, ProtoPipes.fromBindings(boundPipes));
...
function _createElementBinder(directiveResolver, nestedProtoView, elementBinderStack, boundElementIndex, distanceToParentBinder, distanceToParentPei, beginElementCmd) {
    ...
    var directiveBindings = beginElementCmd.directives.map(type => bindDirective(directiveResolver, type));
    ...
    if (directiveBindings.length > 0 || hasVariables || isEmbeddedTemplate) {
        ...
        protoElementInjector = ProtoElementInjector.create(parentProtoElementInjector, boundElementIndex, directiveBindings, isPresent(componentDirectiveBinding), distanceToParentPei, directiveVariableBindings);
        protoElementInjector.attributes = arrayToMap(beginElementCmd.attrNameAndValues, false);
    }
    return new ElementBinder(boundElementIndex, parentElementBinder, distanceToParentBinder, protoElementInjector, componentDirectiveBinding, nestedProtoView);
}
...
export function createDirectiveVariableBindings(variableNameAndValues, directiveBindings) {
    var directiveVariableBindings = new Map();
