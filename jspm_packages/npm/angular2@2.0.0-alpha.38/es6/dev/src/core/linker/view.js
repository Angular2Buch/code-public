/* */ 
"format cjs";
import { MapWrapper, Map, StringMapWrapper } from 'angular2/src/core/facade/collection';
...
export class AppProtoView {
    constructor(templateCmds, type, isMergable, changeDetectorFactory, templateVariableBindings, pipes) {
    ...
    init(render, elementBinders, textBindingCount, mergeInfo, variableLocations) {
        ...
        if (isPresent(this.templateVariableBindings)) {
            MapWrapper.forEach(this.templateVariableBindings, (templateName, _) => { this.protoLocals.set(templateName, null); });
        }
        if (isPresent(variableLocations)) {
            ...
            MapWrapper.forEach(variableLocations, (_, templateName) => { this.protoLocals.set(templateName, null); });
        }
    }
